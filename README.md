...
Проверяется, что имя enum-константы не повторяется.
...
**Сообщение:**
Повторяющееся имя: Red


### 2. Корректность имени типа
Проверяется, что имя перечисления начинается с заглавной буквы.
...
### 3. Корректность имени enum-константы
Проверяется, что имя enum-константы начинается с заглавной буквы.
...
### 4. Проверка пустого перечисления
Проверяется, что перечисление содержит хотя бы одну константу.
...
- EnumDeclarationNode — узел объявления перечисления.
- CaseNode — узел enum-константы.

EnumDeclarationNode Содержит:
- type_name — имя перечисления
- children — список enum-констант

CaseNode содержит:
- name — имя enum-константы
...
```fsharp
└── EnumDeclarationNode (name: Day)
    ├── CaseNode (name: Monday)
    ├── CaseNode (name: Tuesday)
    └── CaseNode (name: Friday)
```
...
Вывод AST осуществляется непосредственно в интерфейсе программы после выполнения анализа.
...
**2.Ошибка: уникальность идентификатора**
![err1](error3.png)

**3.Ошибка: Корректность имени типа**
![err2](error2.png)

**4.Ошибка: Пустое перечисление**
![err3](error1.png)
...
После запуска откроется графический интерфейс текстового редактора.

### Использование программы

1. Ввести корректную конструкцию объявления перечисления F#.
2. Нажать кнопку **«Синтаксический анализ»**.
...
4. При наличии ошибок:

   * ошибки будут выведены в таблице
   * при нажатии на ошибку курсор перейдет к месту ошибки
5. AST будет выведено в отдельной области интерфейса программы.
